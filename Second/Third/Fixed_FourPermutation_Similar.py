import os, json, random, itertools, numpy as np, pandas as pd, tensorflow as tf
...
TRAIN_DS   = "1"
TEST_DS    = "2"  # You can change to any target test dataset
VAL_FRAC   = 0.20
BATCH      = 128
EPOCHS     = 300
PATIENCE   = 10
...
# ---------------- helpers --------------------------------------------------- #
def permute_first_four_bytes(packet: list) -> list:
    """Permute the first 4 bytes randomly to break positional bias."""
    if len(packet) < 4:
        return packet
    first4 = packet[:4]
    random.shuffle(first4)
    return first4 + packet[4:]
...
def load_dataset(ds, L):
    ...
    for cat in ["Telegram", "Non-Telegram"]:
        ...
        for pkt in data["features"]:
            pkt = permute_first_four_bytes(pkt)
            sliced = pkt[:L]
            if len(sliced) != L:
                continue
            xs.append(sliced)
